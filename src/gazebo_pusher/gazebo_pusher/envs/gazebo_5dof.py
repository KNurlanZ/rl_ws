...
class GazeboPlanar5DofEnv(gym.Env):
	...
	def step(self, action):
		...
		dex = a.link_state.pose.position.x
		...
		# 	done = True
		# 	reward = 16
		reward = self.calculate_reward(a.link_state.pose.position.x, contact, self.pos_old)
		dex = a.link_state.pose.position.x
		if (reward == -1 or reward == 0.7):
			done = True
	...
	def states2obs(self,states):
		...
		return result
	def calculate_reward(self, x, contact, old_x):
		if (sum(contact) == 0):
			return -1
		if (x > 3.8):
			return 0.7
